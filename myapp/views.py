...
def confirm_payment(request):
    ...
    if request.method == 'POST':
        ...
        if usertype == 'VVIP':
            ...
            discount_rate = 5  # Fix typo here from 'disoucnt_rate'
        ...
        if payment_method == 'credit_card':
            ...
            if selected_card_id:
                credit_card = get_object_or_404(CreditCard, id=selected_card_id, user=request.user)
                bill.credit_card = credit_card  # Save the selected credit card
                bill.save()
        ...
        for item in items:
            BillItem.objects.create(
                ...
                total_price=item.total_price()  # Correctly call the method here
            )
...
def all_bills(request):
    user_bills = Bill.objects.filter(user=request.user).order_by('-created_at')  # Change 'date' to 'created_at'
...
def bill_detail(request, bill_id):
    ...
    return render(request, 'myapp/bill_detail.html', context)
...
def admin_all_bills(request):
    ...
    if status_filter in ['Paid', 'Cancelled', 'Waiting']:
        bills = Bill.objects.filter(status=status_filter).order_by('-created_at')
    else:
    ...
    return render(request, 'myapp/admin_all_bills.html', {
        'bills': page_obj,
...
def admin_bill_detail(request, bill_id):
    ...
    if request.method == 'POST':
        ...
        if new_status in ['Paid', 'Cancelled', 'Waiting']:
            # If status is 'Paid', update the product stock
            if new_status == 'Paid':
                # Begin a transaction to ensure stock is updated safely
                with transaction.atomic():
                    for item in bill_items:
                        product = item.product
                        if product.quantity >= item.quantity:
                            product.quantity -= item.quantity  # Deduct the quantity sold
                            product.save()
                        else:
                            # Handle insufficient stock situation (optional)
                            return render(request, 'myapp/admin_bill_detail.html', {
                                'bill': bill,
                                'bill_items': bill_items,
                                'error': f"Insufficient stock for {product.title}. Only {product.quantity} left."
                            })
            # Update bill status
